...
export default function RetailerCatalogLayout({
  ...
  return (
    <div className="min-h-screen bg-[#F9F6F4]">
      ...
      <div className="hidden sm:flex h-screen pt-[42px]">
        <Sidebar open={open} setOpen={setOpen}>
          <SidebarBody className="justify-between gap-4">
                ...
                <div className="bg-white/50 rounded-lg p-2 sm:p-3 backdrop-blur-sm">
                  {open ? (
                    ...
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <div className="text-center">
                        <p className="text-xs font-bold text-[#2E1B1B]">{summary.totalSets}</p>
                        <p className="text-xs text-[#4A3A3A]">sets</p>
                      </div>
                      <div className="text-center">
                        <p className="text-xs font-bold text-[#C08478]">₹{summary.totalAmount.toFixed(0)}</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {/* Submit Order Button */}
            {orderItems.length > 0 && (
              <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-[#E5E0DC]">
                <Button
                  onClick={handleSubmitOrder}
                  className={cn(
                    "h-10 sm:h-12 font-semibold bg-gradient-to-r from-[#D9A8A0] to-[#C08478] hover:from-[#C08478] hover:to-[#B0766A] text-white rounded-lg shadow-lg transition-all duration-200",
                    open ? "w-full text-xs sm:text-sm" : "w-10 sm:w-12 text-xs"
                  )}
                  disabled={isSubmittingOrder}
                >
                  {isSubmittingOrder ? (
                    <>
                      <Loader2 className={cn("animate-spin", open ? "w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2" : "w-3 h-3 sm:w-4 sm:h-4")} />
                      {open && "Submitting..."}
                    </>
                  ) : (
                    <>
                      <ShoppingCart className={cn(open ? "w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2" : "w-3 h-3 sm:w-4 sm:h-4")} />
      ...
      {/* Mobile Layout */}
      <div className="sm:hidden flex flex-col min-h-screen pt-[96px]">
        {/* Mobile Header */}
        <div className="bg-white border-b border-[#E5E0DC] p-3 sticky top-[8px] z-40">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="h-8 w-8 rounded-lg bg-gradient-to-br from-[#D9A8A0] to-[#C08478] flex items-center justify-center">
                <FileText className="h-5 w-5 text-white" />
              </div>
              <div>
                <p className="font-bold text-sm text-[#2E1B1B]">{catalog?.catalogName}</p>
            ...
            {orderItems.length > 0 && (
              <Button
                onClick={handleSubmitOrder}
                className="h-8 px-3 text-xs font-semibold bg-gradient-to-r from-[#D9A8A0] to-[#C08478] hover:from-[#C08478] hover:to-[#B0766A] text-white rounded-lg"
                disabled={isSubmittingOrder}
              >
                {isSubmittingOrder ? (
                  <Loader2 className="w-3 h-3 animate-spin" />
                ) : (
                  `Order (${summary.totalSets})`
                )}
              </Button>
            )}
          </div>
  ...
  return (
    <div className="relative z-20 flex items-center space-x-2 py-1 text-sm font-normal text-[#2E1B1B]">
      ...
      >
        ...
        <span className="text-xs text-[#4A3A3A]">
          {catalog?.catalogCode}
        </span>
      </motion.div>
    </div>
